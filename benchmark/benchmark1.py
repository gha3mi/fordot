...
plt.title(f'dot_product Benchmark - Average Elapsed Time')  # Title for the plot
...
plt.xlabel('Number of Elements')  # X-axis label
...
plt.savefig(f'benchmark/benchmark1t.png', dpi=600)  # Save the plot as an image

# Create the second plot for gflops vs size
plt.figure(figsize=(7, 6))  # Create another new figure for the second plot
plt.title(f'dot_product Benchmark - Total Performance')  # Title for the plot
for method, group in benchmark_data.groupby('method'):
...
plt.savefig(f'benchmark/benchmark1p.png', dpi=600)  # Save the plot as an image
