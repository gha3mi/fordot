...
file_name = 'benchmark/benchmark1_im1.data'
...
plt.title(f'Coarray Transpose Benchmark - Average Elapsed Time\nnumber_images={num_images}')  # Title for the plot
...
plt.savefig(f'benchmark/benchmark1t_nim{num_images}.png', dpi=300)  # Save the plot as an image

# Create the second plot for gflops_total vs size
plt.figure(figsize=(7, 6))  # Create another new figure for the second plot
plt.title(f'Coarray Transpose Benchmark - Total Performance\nnumber_images={num_images}')  # Title for the plot
for method, group in benchmark_data.groupby('method'):
...
plt.savefig(f'benchmark/benchmark1p_nim{num_images}.png', dpi=300)  # Save the plot as an image
